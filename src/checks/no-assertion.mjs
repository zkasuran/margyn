  ...
  for (const file of tracked(root)) {
    ...
    for (const { name, span, line } of testCalls(source)) {
      ...
      findings.push({
        ...
        severity: "high",
        file: `${file}:${line}`,
        summary: `test "${name}" asserts nothing, so it cannot fail`,
        ...
        why: "The test runs the code and reports green whatever the code returns. It counts toward coverage and guards nothing.",
      });
    }
  }
  return findings;
